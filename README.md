...
2. `cd` to `tf_pip` or `tf_nopip` depending on desire use TensorFlow installed with pip or installed from `conda-forge` channel.
3. Run `sh setup_conda_tf.sh` in [tf_pip for v2.12](tf_pip) or [tf_nopip for v2.11](tf_nopip) to create a conda environment with TensorFlow and the appropriate libraries and environment variables.
...
5. Activate the environment with `module load conda` and `conda activate tf212gpu` or `conda activate tf211gpu`. 
